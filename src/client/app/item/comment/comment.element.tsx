...
@CustomElement({
    ...
    template: (el: CommentElement) => <template>
        <span ref="toggle" class="comment__toggle">[-]</span>
        <LinkElement class="comment__by" path={`/user/${el.comment.user}`}>{el.comment.user}</LinkElement>
        <LinkElement class="comment__time" path={`/item/${el.comment.id}`}>{el.comment.time_ago}</LinkElement>

        <p ref="text" class="comment__text">{el.comment.content}</p>
        <div class="comment__comments" ref="commentsList">
            {
